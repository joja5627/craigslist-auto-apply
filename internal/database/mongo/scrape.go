...
type Repo interface {
	...
	Add(listing *scrape.Listing) error
...
func (r *MongoRepository) Add(listing *scrape.Listing) error {
	session, coll := r.getSession()
	defer session.Close()
	coll.Insert(listing)
...
func (r *MongoRepository) GetAll()  []*scrape.Listing {
	var listings []*scrape.Listing // see bson.M
	session, coll := r.getSession()
